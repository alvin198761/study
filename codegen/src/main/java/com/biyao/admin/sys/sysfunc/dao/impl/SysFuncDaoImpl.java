package com.biyao.admin.sys.sysfunc.dao.impl;
...
import com.biyao.admin.sys.sysfunc.dao.ISysFuncDao;
 ...
 */
@Repository
public class SysFuncDaoImpl extends BaseDao implements ISysFuncDao {
	private final String insertSql = "INSERT INTO SYS_FUNC (FUNC_ID,NAME,TYPE,IS_LEAF,URL,PARENT_ID,ORDER_CODE,REMARK,DR,TS) VALUES (?,?,?,?,?,?,?,?,?,?) ";
	private final String updateSql = "UPDATE SYS_FUNC SET NAME=?,TYPE=?,IS_LEAF=?,URL=?,PARENT_ID=?,ORDER_CODE=?,REMARK=?,DR=?,TS=? WHERE FUNC_ID=? ";
	...
	public SysFuncDaoImpl() {
		selectSql.append("SELECT T.FUNC_ID,T.NAME,T.TYPE,T.IS_LEAF,T.URL,T.PARENT_ID,T.ORDER_CODE,T.REMARK,T.DR,T.TS FROM SYS_FUNC T WHERE 1=1");
	}
	...
	public String insert(SysFunc vo) {
		...
		// Integer
		// id=jdbcTemplate.queryForObject("SELECT SYS_FUNC_SEQ.NEXTVAL FROM DUAL",
		// Integer.class);
		String id = UUIDGenerator.getUUID();
		Object[] params = new Object[] { id, vo.getName(), vo.getType(), vo.getIs_leaf(), vo.getUrl(), vo.getParent_id(), vo.getOrder_code(), vo.getRemark(),
				vo.getDr(), vo.getTs() };
	...
	public int update(SysFunc vo) {
		Object[] params = new Object[] { vo.getName(), vo.getType(), vo.getIs_leaf(), vo.getUrl(), vo.getParent_id(), vo.getOrder_code(), vo.getRemark(),
				vo.getDr(), vo.getTs(), vo.getFunc_id() };
	...
	public Map<String, Object> queryList(SysFuncCond cond, Map<String, Object> map) {
		StringBuilder sb = new StringBuilder(selectSql);
		sb.append(cond.getCondition() + " ORDER BY T.FUNC_ID");
		logger.debug(DataUtil.showSql(sb.toString(), cond.getArray()));// 显示SQL语句
		// return page.getPage(map, sb.toString(), curPage, pageSize,
		// cond.getArray(), SysFunc.class);//(使用范型)
	...
	public int deleteLogic(String ids) {
		String updateStr = "UDATE SYS_FUNC SET DR = 1 WHERE FUNC_ID" + Util.ArrayToIn(ids);
		return jdbcTemplate.update(updateStr);
	}
