...
public class JspList {
	...
	private static StringBuilder htmlTitle(String CNName, String className, int i18n) {
		...
		sb.append("\r\n\t\t\t\t<td class=\"bu\">");
		sb.append("\r\n\t\t\t\t\t<input <%--<u:limit key=\"按钮关键字\"/>--%> type=\"button\" icon=\"icon-excel\" value=\"" + I18nUtil.getValue(5, i18n) + "\" onclick=\"toImport();\" />");
		sb.append("\r\n\t\t\t\t\t<input type=\"button\" icon=\"icon-excel\" value=\"" + I18nUtil.getValue(4, i18n) + "\" onclick=\"toAction('${webPath}/" + className + "/export');\" />");
		sb.append("\r\n\t\t\t\t\t<input type=\"button\" icon=\"icon-add\" value=\"" + I18nUtil.getValue(7, i18n) + "\" onclick=\"toAction('${webPath}/" + className + "/toAdd');\" />");
	...
	private static StringBuilder htmlJS(String className, StringBuilder clear, String CNName) {
		...
		sb.append("\r\n\t\tvar url = \"${webPath}/common/importExcel?action=" + className + "/import&title=" + CNName + "Excel导入\";");
