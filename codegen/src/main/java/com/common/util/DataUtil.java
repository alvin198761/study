...
public class DataUtil {
	 ...
	 */
	public static void export(String[] headers, List<Map<String, Object>> list, HttpServletResponse response, String title, String operator) throws Exception {
		response.reset();
		response.setContentType("application/vnd.ms-excel");
		response.setHeader("Content-disposition", "attachment;filename=" + new String((title + ".xls").getBytes(), "iso8859-1"));
	...
	public static void addsheet(String[] headers, List<Map<String, Object>> list, String title, String operator, WritableWorkbook wbook, int sheetIndex) throws Exception {
		...
		for (Map<String, Object> map : list) {
			...
			for (Map.Entry<String, Object> entry : map.entrySet()) {
				wsheet.addCell(new Label(columnIndex++, rowIndex, null != entry.getValue() ? entry.getValue().toString() : "", format));
			}
		}
		// 设置表头信息与样式
	...
	/**
	 * 批量导入指定类型的List(自增涨型的主键,不需给出主键,对应的INSERT中也没有PK字段)
	 * 
	 * @param list
	 * @param jdbcTemplate
	 * @param sql
			...
			public void setValues(PreparedStatement ps, int i) {
				try {
					...
					for (int j = 1; j <= fields.length; j++) {
						...
						} else if (fields[j - 1].getType().getName().contains("BigDecimal") && null != method.invoke(t)) {
	...
	public static <T> String showSql(String sql, T t) {
		...
		try {
			for (int j = 0; j < fields.length; j++) {
				param = "有情况";
				method = t.getClass().getDeclaredMethod("get" + firstUpper(fields[j].getName()));
				if (null != method.invoke(t)) {
					String cname = fields[j].getType().getName();
					if (cname.contains("Integer")) {
						param = Integer.toString((Integer) method.invoke(t));
					} else if (cname.contains("Date")) {
						param = "'" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(method.invoke(t)) + "'";
					} else if (cname.contains("String")) {
						param = "'" + (String) method.invoke(t) + "'";
					} else if (cname.contains("Long")) {
						param = Long.toString((Long) method.invoke(t));
					} else if (cname.contains("Float")) {
						param = Float.toString((Float) method.invoke(t));
					} else if (cname.contains("Boolean")) {
						param = Boolean.toString((Boolean) method.invoke(t));
					} else if (cname.contains("BigDecimal")) {
						param = (String) method.invoke(t);
					} else {
						param = "'" + (String) method.invoke(t) + "'";
					}
		...
		for (int j = 0; null != obj && j < obj.length; j++) {
			...
			if (null != obj[j]) {
				...
				if (cname.contains("Date") || cname.contains("Timestamp")) {
					param = "convert(datetime,'" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(obj[j]) + "')";
				} else if (cname.contains("String")) {
					param = "'" + (String) obj[j] + "'";
				} else {
	...
	public static void main(String[] args) {
		Object[] obj1 = { " 123  ", 456, "789  ", new Date(), new Timestamp(new Date().getTime()), new Timestamp(new Date().getTime()), new Date(), new BigDecimal(1), new Float(0), new Double(0),
				new Long(1), "   wioasderu   ", "   wioerasda4e23u   " };

		String sqlString = "????????????";
		showSql(sqlString, obj1);
		logger.info(showSql(sqlString, obj1));
	}
}
