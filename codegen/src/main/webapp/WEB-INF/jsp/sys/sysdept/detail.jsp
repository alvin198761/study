...
<title>组织机构详细<%--author:高振中,DATE:2014-07-29 16:49:42--%></title>
</head>
<script type="text/javascript">
	...
	function getSysDeptRef(name1, name2, path) {
		...
		url = path + "/sysdept/ref?rname=" + name1 + "&rvalue=" + name2 + "&type=radio";
		winWidth = 500;
		winHeight = $(top.window).height() - 20;
		widId = 'win001';
		top.ShowWin('组织机构详细', url, path, winWidth, winHeight, widId);
	}
</script>
<body>
	...
	<table class="titleTab">
		<tr>
			<td class="ti">组织机构详细</td>
			<td class="bu">
				<input type="button" icon="icon-add" onclick="toAction('${webPath}/sysdept/toAdd?id=${sysdept.id}')" value="新增下级"/> 
				<input type="button" icon="icon-edit" onclick="toAction('${webPath}/sysdept/toUpdate?id=${sysdept.id}')" value="编辑"/> 
	...
	<table class="dataTab">
		...
		<tr>
			...
			<td><fmt:formatDate value="${sysdept.ts}" type="both" /></td>
			<%--type取值还有date,time,中文年月日时分秒等--%>
		</tr>
		<tr>
			<td class="right30_blue">删除标志</td>
			<td>${sysdept.dr}</td>
		</tr>
	</table>
<%-- 		<div class="ti">参照页范例</div>
		<table class="dataTab">
			<tr>
				<td class="right30">组织机构详细</td>
				<td>
					<input id="anyname" readonly="readonly" onClick="getSysDeptRef('anyname','anyid','${webPath}');" class="inputref" />
					<input id="anyid" />
				</td>
			</tr>
		</table> --%>
