...
<title>资产类型表列表<%--author:高振中,DATE:2014-07-29 16:08:38--%></title>
</head>
<script type="text/javascript">
	function toDelete(ids) {//删除记录
		if(checkDel(ids)){
			toAction("${webPath}/systype/delete?id=" + ids);
		}
	}
	function toClear() {//清空查询条件
		clearCond(['name_c','parent_id_c','is_leaf_c','ts_c','remark_c','order_code_c']);
	}
	function toDetail(id) {//查看单条记录详细
		top.ShowWin('详细页面','${webPath}/systype/detail?id='+id,'${webPath}',600,600,'win001');
	}
</script>
<body>
	<form:form id="form1" method="post" commandName="cond">
		<div class="ti">资产类型表查询</div>
		<table class="SearchTab">
			<tr>
				<td align="right">名称</td>
				<td><form:input path="name_c" class="text-m" /></td>
				<td align="right">父结点ID</td>
			...
			<tr>
				...
				<td></td>
				<td>
					<input type="button" icon="icon-search" value="查询" onclick="toAction('${webPath}/systype/list');" /> 
		...
		<table class="titleTab">
			<tr>
				<td class="ti">资产类型表列表</td>
				<td class="bu">
					<input type="button" icon="icon-add" value="新增" onclick="toAction('${webPath}/systype/toAdd?id=${cond.parent_id_c}');" />
					<input type="button" icon="icon-delete" value="删除" onclick="toDelete(getIds());" />
				</td>
			</tr>
		</table>
		<table class="dataTab">
			...
			<c:forEach items="${dataList}" var="wz" varStatus="vs">
				<tr>
					...
					<td class="td-upd" onclick="toAction('${webPath}/systype/toUpdate?id=${wz['id']}');"></td>
