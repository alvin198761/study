...
<script type="text/javascript">
	...
	function exportToExcel() {//导出到Excel
		form1.action = "${webPath}/user/export";
		form1.submit();
	}
	function importFromExcel() {//从Excel导入列表
		var text, url, webpath, winWidth, winHeight, widId, action;
		text = "";
		action = "${webPath}/user/import";
		webpath = "${webPath}";
		url = "${webPath}/common/importExcel?action="+action+"&title=用户Excel导入";
		winHeight = 180;
		winWidth = 650;
		widId = 'win001';
		top.ShowWin(text, url, webpath, winWidth, winHeight, widId);
	...
	<form:form name="form1" method="post" commandName="cond">
		...
		<table class="SearchTab">
			<tr>
				...
				<td><form:input path="e_mail_c" /></td>
			</tr>
			<tr>
		...
		<table class="titleTab">
			<tr>
				<td class="ti">用户列表</td>
				<td class="bu">
					<input type="button" icon="icon-excel" value="导入EXEL" onclick="importFromExcel()" />
					<input type="button" icon="icon-excel" value="导出EXEL" onclick="exportToExcel()" />
