...
function checkAll() {
	$(":checkbox[name='chk']").each(function() {
		$(this).attr("checked", document.getElementById("chkAll").checked);
	});
}
/*******************************************************************************
 * 作者:TY
...
//表格查询
function queryData() {
	var params = $('#dataTab').datagrid('options').queryParams; //先取得 datagrid 的查询参数
	var fields = $('#form1').serializeArray(); //自动序列化表单元素为JSON对象
	$.each(fields, function(i, field) {
		params[field.name] = field.value; //设置查询参数
	});
	$('#dataTab').datagrid('reload'); //设置好查询参数 reload 一下就可以了
}
//清空查询条件
function clearCond() {
	$('#form1').form('clear');
