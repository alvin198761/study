...
public class ClassCondition {
	public static StringBuilder genSB(String packname, String className, List<Field> list, String author, String CNName, Integer isWeb) {
		...
		for (Field fieldname : list) {
			name = fieldname.getName();
			comments = fieldname.getComments();
			Cname = name.toLowerCase();
			if (isWeb == 1) {// 如果是web工程
				Cname += "_c";
			}
			type = fieldname.getType();
			condtion = fieldname.getCond();
